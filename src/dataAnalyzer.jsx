...
export function UrlCell({ value }) {
  ...
  const handleClick = (e) => {
    ...
    const dontShowAgain = sessionStorage.getItem(
      "datagrid-url-warning-dismissed",
    );
      ...
      <Dialog open={showWarning} onOpenChange={setShowWarning}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to leave the application and visit an external
              website:
              <br />
              <span className="font-mono text-xs mt-2 block bg-muted p-2 rounded break-all">
                {value}
              </span>
            </DialogDescription>
          </DialogHeader>
...
function detectDataType(values, columnName) {
  ...
  const isPhoneColumn = phoneKeywords.some((keyword) =>
  ...
  if (
    values
      .slice(0, 10)
      .every((val) => typeof val === "string" && datePattern.test(val))
  ) {
    return "date";
  }
  ...
  if (numericValues.length / values.length > 0.8) {
    ...
    if (
      values.some((val) => typeof val === "string" && currencyPattern.test(val))
    ) {
      return "currency";
    }

    if (values.some((val) => typeof val === "string" && val.includes("%"))) {
      return "percentage";
    }
...
export function getCellRenderer(dataType, isEnum, uniqueValues = []) {
  ...
  if (dataType === "number") {
    return ({ getValue, column }) => {
      ...
      const config = getColumnConfig(column.id);
      const precision =
        config?.precision !== undefined ? config.precision : undefined;
